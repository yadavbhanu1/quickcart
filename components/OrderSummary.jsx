...
const OrderSummary = ({ onPlaceOrder }) => {
  ...
  // Pass selectedAddress up to parent
  const handlePlaceOrderClick = () => {
    if (!selectedAddress) {
      toast.error("Please select a delivery address before placing order.");
      return;
    }
    onPlaceOrder(selectedAddress); // pass address to parent
  };
  ...
  const itemsAmount = getCartAmount();
  const taxAmount = Math.floor(itemsAmount * 0.02);
  ...
  return (
    <div className="w-full md:w-96 bg-gray-500/5 p-5 rounded-md">
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="relative inline-block w-full text-sm border rounded">
            ...
            {isDropdownOpen && (
              <ul className="absolute w-full bg-white border shadow-md mt-1 z-10 py-1.5 max-h-60 overflow-auto rounded">
      ...
      <button
        ...
        className="w-full bg-orange-600 text-white py-3 mt-5 rounded hover:bg-orange-700"
      >
        Place Order
      </button>
    </div>
